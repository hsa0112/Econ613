...
ex1Data <- datind2009.csv[,c("idind","idmen","empstat","wage","age")] %>% filter(!is.na(wage),!is.na(age), wage != 0, age >= 18)
...
for (i in 1:49){
  sampleIndices <- sample(nrow(ex1Data),nrow(ex1Data),replace = TRUE) # randomly choose 500 data (with replacement)
...
ex2Data <- ex2Data %>% filter(!is.na(age),!is.na(wage), wage != 0, age >= 18)
...
groupNames <- c(rep("G1",14),rep("G2",14),rep("G3",14),rep("G4",14),rep("G5",14),rep("G6",14),rep("G7",14),rep("G8",14),rep("G9",14))
...
```{r}
#a
ex3Data <- datind2007.csv %>% filter(empstat != "Inactive", empstat != "Retired", age >= 18)
ex3Data
```
...
ex4Data <-  ex4Data %>% filter(empstat != "Inactive",!is.na(empstat), empstat != "Retired", age >= 18)
```
...
probitFormula <- dataMeans * as.numeric(probitCoef)
logitFormula <- dataMeans * as.numeric(logitCoef)
...
memProbit <- dnorm(sum(probitFormula)) * as.numeric(probitCoef[2:length(probitCoef)])
memProbit[1]


#MEM is dnorm of the Formula above (sum to get them to add) times the corresponding coefficients to get MEM of each variable (age and year dummies)
paste("Logit")
memLogit <- exp(sum(-logitFormula))/(1+exp(-sum(logitFormula)))^2 * as.numeric(logitCoef[2:length(logitCoef)])
memLogit[1]
...
for (p in 1:49){
  indices <- sample(nrow(ex4Data),nrow(ex4Data),replace = TRUE)
  ...
  memProbitBS[p,] <- dnorm(sum(dataMeansBS * probitParam)) * as.numeric(probitCoef[2:length(probitParam)])
  memLogitBS[p,] <- exp(-sum(logitFormula))/(1+exp(sum(-logitFormula)))^2 * as.numeric(logitCoef[2:length(logitParam)])
...
for (c in 1:length(memProbitBS_MEM_Var)){
