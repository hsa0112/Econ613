...
#     
  ...
  for (i in 1:nrow(migratedHouseData)){
    ID <- migratedHouseData[i,"idind"]
    occ <- which(migratedHouseData[,"idind"] == ID)
    if (length(occ) > 1){
      currentIndex <- which(occ == i)
      prevIndex <- currentIndex - 1
      if (!is.na(migratedHouseData[currentIndex,"profession"]) & !is.na(migratedHouseData[prevIndex,"profession"] )){
        if(migratedHouseData[currentIndex,"profession"] != ""  & migratedHouseData[prevIndex,"profession"] != ""){
          migratedHouseData[currentIndex,"Change"] <- 1
        }
      }
    }
